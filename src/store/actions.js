...
export const anthologyReceive = ({ state, commit, dispatch }, anthology) => {
  console.log('收到了anthology', anthology)
  let author;
  let content = [];
  ...
  } else if (typeof anthology === 'object') {
    console.log('接收到的是对象anthology')
    author = {};
    if (anthology.author.id !== -1) {
      author = {
        ...anthology.author,
      }
    }
    anthology.author = anthology.author.id;
    if (anthology.article && anthology.article.length > 0) {
      anthology.article.map((item) => {
        let obj = {
          ...item,
  ...
  console.log('范式化第一步，分发', anthology, author);
